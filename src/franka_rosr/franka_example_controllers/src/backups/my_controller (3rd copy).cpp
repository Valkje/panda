...
void MyController::update(const ros::Time& /*time*/,
  ...
  if (state.compare("Docked") == 0){
    ...
    double setpoint = -4.0;
    double force_error = setpoint + force_EE_z; // force_EE_z is positive
    force_error_integral_ += period.toSec() * force_error;
    forces(2) = setpoint + k_p_ * force_error + k_i_ * force_error_integral_;
  ...
  //   // std::cout << (-cartesian_stiffness_ * error - cartesian_damping_ * (jacobian * dq));
  // }
  // counter = (counter + 1) % 500;
