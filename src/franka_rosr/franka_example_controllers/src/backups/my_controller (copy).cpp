...
void MyController::update(const ros::Time& /*time*/,
  ...
  if (state.compare("Docked") == 0){
    ...
    // compute control
    // allocate variables
    Eigen::VectorXd tau_task(7), tau_nullspace(7), tau_d_impedance(7);
    ...
    // Cartesian PD control with damping ratio = 1
    tau_task << jacobian.transpose() *
                    (-cartesian_stiffness_ * error - cartesian_damping_ * (jacobian * dq)); // <- force
    // nullspace PD control with damping ratio = 1
    tau_nullspace << (Eigen::MatrixXd::Identity(7, 7) -
                      jacobian.transpose() * jacobian_transpose_pinv) *
                         (nullspace_stiffness_ * (q_d_nullspace_ - q) -
    ...
    desired_force_torque(2) = desired_mass_ * -9.81 + 10*std::abs(error(4));
    ...
    tau_cmd << saturateTorqueRate(tau_cmd+tau_d_impedance, tau_J_d);
  ...
  if (counter % 500 == 0) {
    ...
    std::cout << "Cartesian stiffness target:\n" << cartesian_stiffness_target_ << "\n";
